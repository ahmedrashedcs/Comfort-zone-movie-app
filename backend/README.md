...
Your API will require the creation of one table for each of the data files provided, plus 3 additional database tables (at minimum) to store the user specific data. Each of the user specific tables should have an autonumber primary key.
...
The data for the movies/genres/production companies has been provided on Blackboard (note: the movie_genres file defines the many-to-many relationship between movies and genres, and therefor needs to be it's own table. movies_companies serves the same purpose). I purposely didn't put the .csv file in the repo. Please don't add it.

After importing the data, you must take the time to modify the tables to set/convert to meaningful column types, setup keys and indexes, and define the foreign key relationships (i.e good database design...there will be marks for this)
